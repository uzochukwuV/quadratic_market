use {
    ...
    solana_zk_token_sdk::zk_token_proof_instruction::ZkProofData,
};
...
fn decode_proof_instruction<T, U>(
    ...
    U: Pod,
{
    ...
    ProofInstruction::proof_data(&instruction.data).ok_or(ProgramError::InvalidInstructionData)
}
...
fn process_configure_account(
    ...
    let proof_data = decode_proof_instruction::<PubkeyValidityData, PubkeyValidityProofContext>(
        ProofInstruction::VerifyPubkeyValidity,
        &previous_instruction,
    )?;

    if proof_data.context.pubkey != *encryption_pubkey {
...
fn process_empty_account(
    ...
    let previous_instruction =
        get_instruction_relative(proof_instruction_offset, instructions_sysvar_info)?;
    let proof_data = decode_proof_instruction::<ZeroBalanceProofData, ZeroBalanceProofContext>(
    ...
    if confidential_transfer_account.available_balance != proof_data.context.ciphertext {
        msg!("Available balance mismatch");
        return Err(ProgramError::InvalidInstructionData);
    }

    if confidential_transfer_account.encryption_pubkey != proof_data.context.pubkey {
        return Err(TokenError::ConfidentialTransferElGamalPubkeyMismatch.into());
    }
...
fn process_withdraw(
    ...
    let proof_data = decode_proof_instruction::<WithdrawData, WithdrawProofContext>(
...
fn process_transfer(
    ...
    if let Ok(transfer_fee_config) = mint.get_extension::<TransferFeeConfig>() {
        ...
        let proof_data =
            decode_proof_instruction::<TransferWithFeeData, TransferWithFeeProofContext>(
                ProofInstruction::VerifyTransferWithFee,
                &previous_instruction,
            )?;

        if proof_data.context.transfer_with_fee_pubkeys.auditor
        ...
        if proof_data.context.transfer_with_fee_pubkeys.withdraw_withheld_authority
        ...
        if u64::from(proof_data.context.fee_parameters.maximum_fee) != maximum_fee
            || u16::from(proof_data.context.fee_parameters.fee_rate_basis_points)
                != transfer_fee_basis_points
        {
        ...
        let source_ciphertext_lo = EncryptedBalance::from((
            proof_data.context.ciphertext_lo.commitment,
            proof_data.context.ciphertext_lo.source_handle,
        ));
        let source_ciphertext_hi = EncryptedBalance::from((
            proof_data.context.ciphertext_hi.commitment,
            proof_data.context.ciphertext_hi.source_handle,
        ));
        ...
        process_source_for_transfer(
            ...
            authority_info,
            account_info_iter.as_slice(),
            &proof_data.context.transfer_with_fee_pubkeys.source,
        ...
        let destination_ciphertext_lo = EncryptedBalance::from((
            proof_data.context.ciphertext_lo.commitment,
            proof_data.context.ciphertext_lo.destination_handle,
        ));
        let destination_ciphertext_hi = EncryptedBalance::from((
            proof_data.context.ciphertext_hi.commitment,
            proof_data.context.ciphertext_hi.destination_handle,
        ));

        process_destination_for_transfer(
            destination_token_account_info,
            mint_info,
            &proof_data.context.transfer_with_fee_pubkeys.destination,
        ...
        let proof_data = decode_proof_instruction::<TransferData, TransferProofContext>(
            ProofInstruction::VerifyTransfer,
            &previous_instruction,
        )?;

        if proof_data.context.transfer_pubkeys.auditor
            != confidential_transfer_mint.auditor_encryption_pubkey
        {
            return Err(TokenError::ConfidentialTransferElGamalPubkeyMismatch.into());
        }
        ...
        process_source_for_transfer(
            ...
            authority_info,
            account_info_iter.as_slice(),
            &proof_data.context.transfer_pubkeys.source,
            &source_ciphertext_lo,
            &source_ciphertext_hi,
            new_source_decryptable_available_balance,
        )?;
        ...
        process_destination_for_transfer(
            destination_token_account_info,
            mint_info,
            &proof_data.context.transfer_pubkeys.destination,
            &destination_ciphertext_lo,
            &destination_ciphertext_hi,
            None,
        )?;
...
fn process_withdraw_withheld_tokens_from_mint(
    ...
    let _ = (program_id, accounts, proof_instruction_offset);
...
fn process_withdraw_withheld_tokens_from_accounts(
    ...
    let _ = (program_id, accounts, num_token_accounts, proof_instruction_offset);
    Err(ProgramError::InvalidInstructionData)
}
...
fn harvest_from_account<'a, 'b>(
    ...
    if !confidential_transfer_account.withheld_amount.is_zeroed() {
        let transfer_fee_amount = token_account.get_extension_mut::<TransferFeeAmount>()?;
        let new_withheld_amount = ops::add(
            &transfer_fee_amount.withheld_amount.into(),
            &confidential_transfer_account.withheld_amount,
        )
        .ok_or(ProgramError::InvalidInstructionData)?;
        transfer_fee_amount.withheld_amount = new_withheld_amount.into();
    }

    Ok(confidential_transfer_account.withheld_amount)
}
